    ...
    if enrollment_type == "Regular":
        ...
        if "record_semester_key" in st.session_state:
            ...
            program = st.selectbox(
                ...
                ["JD", "BSED-English", "BSED-Math", "BSCS"],
            ...
            with col1:
                record_program = st.selectbox(
                    "Program to Record",
                    ["JD", "BSED-English", "BSED-Math", "BSCS"],
                    index=["JD", "BSED-English", "BSED-Math", "BSCS"].index(program),
...
with tab2:
    ...
    if enrollments:
        ...
        with col2:
            st.metric("Regular Students", len(df[df["Status"] == "Regular"]))
        with col3:
            st.metric("Irregular Students", len(df[df["Status"] == "Irregular"]))
    else:
