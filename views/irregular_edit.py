...
def show():
    ...
    # -------------------------
    if 'selected_student_id1' not in st.session_state:
        st.session_state.selected_student_id1 = None
    ...
    student_df = student_df[student_df["status"].str.lower() == "irregular"]
    ...
    with col2:
        if st.button("🔍 Load Student"):
            if selected_student_name:
                if st.session_state.selected_student_id1 != name_to_id[selected_student_name]:
                    st.session_state.selected_semester = None  # Reset only when switching student
                st.session_state.selected_student_id1 = name_to_id[selected_student_name]
                st.rerun()

    # -------------------------
    # Stop if no student selected yet
    # -------------------------
    if not st.session_state.selected_student_id1:
        st.stop()
    ...
    selected_student = student_df.loc[st.session_state.selected_student_id1]
